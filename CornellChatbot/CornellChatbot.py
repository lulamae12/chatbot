...
epochs = 100 #an iteration over all of the training data
batch_size = 128
rnn_size = 512
num_layers = 2
...
learning_rate = 0.005
learning_rate_decay = 0.9#better to decrease the learning rate as model trains
...
#as stated below this can help with asethithic(however you spell it...)
def pad_sentence_batch(sentence_batch, vocab_to_int):
    """Pad sentences with <PAD> so that each sentence of a batch has the same length"""
    max_sentence = max([len(sentence) for sentence in sentence_batch])
    return [sentence + [vocab_to_int['<PAD>']] * (max_sentence - len(sentence)) for sentence in sentence_batch]#adds the sentence plus a pad of vocab and muliplys it by the sum of the max sentence length vs the actual sentence lenghth.
...
# Split the questions and answers into training data and validation data
...
print(f"Trained questions length:",len(train_questions))
print(f"Valid questions length:",len(valid_questions))
